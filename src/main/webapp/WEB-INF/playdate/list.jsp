        ...
        <tbody>
            ...
            <tr>
                <td>
                    ...
                    <c:choose>
                        <c:when test="${record.eventName.length() == 0}">Playdate @</c:when>
                        <c:otherwise>${record.eventName} @</c:otherwise>
                    </c:choose>
                    <c:choose>
                        <c:when test="${record.locationName.length() > 0}">${record.locationName}</c:when>
                        <c:when test="${record.userMdl.homeName.length() > 0}">${record.userMdl.homeName}</c:when>
                        <c:otherwise>${record.userMdl.userName} </c:otherwise>
                    </c:choose>
                    </a>
                </td>
                <td>
                    <a class="text-decoration-none" href="/profile/${record.userMdl.id}">${record.userMdl.userName}</a>
                </td>
                <td>
                    ${record.playdateStatus.displayValue}
                </td>
                <td>
